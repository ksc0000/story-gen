...
describe("resolveReplicateModel", () => {
  it("uses flux-2-klein-9b for light book pages and covers", () => {
    ...
    expect(resolveReplicateModel({ purpose: "memory_key_page", imageQualityTier: "light" })).toBe(
      "black-forest-labs/flux-2-klein-9b"
    );
  });

  it("uses flux-2-klein-9b for standard book pages and covers", () => {
    expect(resolveReplicateModel({ purpose: "book_page", imageQualityTier: "standard" })).toBe(
      "black-forest-labs/flux-2-klein-9b"
    );
    expect(resolveReplicateModel({ purpose: "book_cover", imageQualityTier: "standard" })).toBe(
      "black-forest-labs/flux-2-klein-9b"
    );
  });

  it("keeps child avatar generation on flux-2-pro", () => {
