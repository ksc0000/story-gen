...
export class GeminiClient implements LLMClient {
  ...
  async rewriteStoryText(params: {
    ...
    const rewriteInstruction = [
      ...
      "Create natural read-aloud rhythm by varying sentence lengths (mixing short and long sentences) on the same page.",
      "Use phrase-level spacing (分かち書き) using half-width spaces to help parents read with natural pauses, but never break words internally (e.g., avoid \"あそ び\").",
      "For ages 5+, include logical connections (cause and effect) to add depth to the narrative.",
      "For ages 5+, use narrative connectives such as 「すると」「でも」「やがて」「ところが」「そのとき」 to create story flow and progression within and between pages.",
