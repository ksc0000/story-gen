...
const AGE_READING_PROFILES: Record<AgeBand, AgeReadingProfile> = {
  baby_toddler: {
    ...
    kanjiPolicy: "漢字は一切使わず、すべてひらがなとカタカナのみで記述してください。",
  ...
  preschool_3_4: {
    ...
    kanjiPolicy: "漢字は原則使わず、ほぼすべてひらがなで記述してください。助詞の『は』『を』『へ』などを正しく使いつつ、読みやすさを最優先してください。",
    storyComplexity: "はじまり、できごと、気持ちの変化、うれしい終わり",
    narrativeComplexity: "「場所・行動・気持ち・発見」のうち2つ以上を含む、意味の通る短い物語文",
    dialoguePolicy: "短い会話を1つまで自然に入れてよい",
    emotionalDepth: "うれしい、びっくり、かなしい、ほっとした、など",
    backgroundDetailLevel: "背景に場所らしさや身近な小物を入れ、どこで何が起きているか分かるようにする",
    repetitionPolicy: "音遊びではなく、意味が進むくり返しを使う",
  ...
  early_reader_5_6: {
    ...
    kanjiPolicy: "ひらがな・カタカナを主体にし、漢字はごく簡単なもの（小学校1年生レベル）を数個までに留めてください。",
    storyComplexity: "小さな問題、挑戦、工夫、気持ちの変化",
    narrativeComplexity: "原因と結果、少しの会話、情景描写を含む意味の通る物語文（場所・行動・気持ち・発見の2つ以上を必須とする）",
    dialoguePolicy: "短い会話を自然に入れてよい",
  ...
  early_elementary_7_8: {
    ...
    kanjiPolicy: "ひらがな中心ですが、小学校低学年向けの簡単な漢字を適切に混ぜてください。難しい漢字は避け、どうしても必要な場合は必ずひらがなで記述してください。",
    storyComplexity: "理由、選択、サブキャラ、小さな葛藤、伏線を少し含める",
    narrativeComplexity: "小さな葛藤、伏線と回収、感情の理由を入れて読みごたえを出す（場所・行動・気持ち・発見の2つ以上を必須とする）",
    dialoguePolicy: "会話を自然に入れてよい",
    emotionalDepth: "迷う、考える、決める、相手を思う、成長する、など",
    backgroundDetailLevel: "背景に意味のある小物、季節感、サブキャラの動き、伏線になる情報を入れる",
  ...
  general_child: {
    ...
    kanjiPolicy: "ひらがな・カタカナを主体にし、漢字は原則として使用しないでください。",
    storyComplexity: "できごと、気持ちの変化、小さな発見が分かりやすい構成",
    narrativeComplexity: "場所・行動・気持ち・発見のうち2つ以上を含む、標準的な絵本の読みごたえ",
...
export function getAgeReadingProfile(age?: number): AgeReadingProfile {
  ...
  // Age > 8: use the most sophisticated profile available (7-8 years old)
  // to ensure naturalness and depth while remaining within picture-book norms.
  return AGE_READING_PROFILES.early_elementary_7_8;
}
