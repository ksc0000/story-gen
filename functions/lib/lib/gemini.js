...
function normalizeStoryCharacterKind(value, role) {
    if (typeof value === "string") {
        const normalized = value.trim().toLowerCase();
        const allowed = [
            "human_child",
    ...
    if (role === "animal")
        return "animal";
    if (role === "object_character")
        return "object_character";
    if (role === "background_recurring")
        return "background";
    if (role === "magical_friend" || role === "buddy")
...
function validateStoryCast(data) {
    ...
    return data.map((entry, index) => {
        ...
        return {
            ...
            negativeCharacterRules: validateStringArray(obj.negativeCharacterRules, `cast[${index}].negativeCharacterRules`),
