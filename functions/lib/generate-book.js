...
function buildFailureMetadata(data) {
    return {
        ...(0, firestore_sanitize_1.removeUndefinedDeep)(data),
        ...createFailedAtPatch(),
    };
}
function sanitizeStoryCastForFirestore(cast) {
    ...
    const sanitized = cast
        .filter((character) => Boolean(character?.characterId) &&
        Boolean(character?.displayName) &&
        Boolean(character?.role) &&
        Boolean(character?.visualBible))
        .map((character) => (0, firestore_sanitize_1.removeUndefinedDeep)({
...
function sanitizeForbiddenQuestObjects(forbiddenQuestObjects, bookData, mergedInput) {
    ...
    const signatureTokens = new Set((signatureItem ?? "")
        .split(/[、,\s]+/)
    ...
    const sanitized = forbiddenQuestObjects.filter((value, index, array) => {
        ...
        if (!normalized)
            return false;
        if (array.findIndex((item) => item.trim() === normalized) !== index)
            return false;
        if (genericForbidden.has(normalized))
            return false;
...
function resolveScenePolicy(bookData, template) {
