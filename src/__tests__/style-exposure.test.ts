...
describe("style exposure config", () => {
  ...
  it("treats known but unlisted pairings as internal and non-selectable", () => {
    const exposure = getStyleTemplateExposure(
      "fixed-first-zoo-8p",
      "toy_3d"
    );
  ...
  it("returns only user-selectable entries for a template", () => {
    const entries = getUserSelectableStyleExposureEntries("fixed-first-zoo-8p");

    expect(entries.map((entry) => entry.styleId)).toEqual([
      "crayon",
      "soft_watercolor",
    ]);
  });
