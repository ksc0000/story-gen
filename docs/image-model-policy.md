...
> 他全候補を上回る。
> → T6-42 で OpenAI Image Client 実装設計、T6-43 で I1 smoke（imagination × crayon）を実施。
...
| primary モデル | flux-2-pro (`pro_consistent`) — 変更なし |
| Replicate inquiry | ❌ 未送付（期限: 2026-05-25）|
| 次フェーズ | T6-43: OpenAIImageClient 最小実装 + I1 smoke（imagination × crayon） |


---

### T6-43: OpenAI Image Client Minimal Implementation (2026-05-18)
...
| `OPENAI_API_KEY` Secret Manager 登録 | ❌ **未登録（404 NOT_FOUND）** |
| functions deploy | ❌ blocked |
| I1 smoke 実行 | ❌ blocked |

**I1 Smoke 状態**: **BLOCKED — human action required**

> `OPENAI_API_KEY` が Firebase Secret Manager に未登録。
> Organization Verification (A3) + API key 登録 (A4) → deploy → smoke 実行の順で解除。
