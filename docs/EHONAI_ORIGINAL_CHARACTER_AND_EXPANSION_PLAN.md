...
## 7-4-1. プロフィール生成と絵本ページ生成の制約は分離する

キャラクタープロフィール画像を作るときの prompt 制約と、絵本本文ページを作るときの prompt 制約は同じにしない。

- プロフィール生成では、背景固定、正面寄り、距離固定などの強い制約を使ってもよい
- 絵本本文ページでは、背景固定よりも `cast`、`visualBible`、`signatureItems`、`doNotChange`、`reference image` で一貫性を担保する
- つまり、キャラ一貫性は「毎回同じ背景に閉じ込める」ことで守るのではなく、「同じ存在としての視覚アンカーを固定する」ことで守る

この分離により、通常の絵本ではページごとに自然な情景変化を許しながら、キャラクターだけは同じ存在として見せられる。
