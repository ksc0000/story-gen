...
- Gemini は **絵本1冊分の story JSON をまとめて1回で生成** する
- 画像生成は **各ページごとに Replicate を呼ぶ**
- そのため、ページ間の主人公の揺らぎは主に画像生成側で改善する
- 参照画像、model profile、prompt の複雑さ、構図、ページごとの visual role が一貫性に強く影響する
...
- `pages[].imagePrompt` は **wordless illustration 専用** として扱い、本文テキスト・繰り返しフレーズ・ラベル文言を渡さない
...
### Klein 向けと Pro 向けで prompt を出し分ける

画像モデルごとの得意・不得意に合わせて、`buildImagePrompt()` は将来的に次のように分ける前提で整備する。

- `klein_fast` / `klein_base`
  - prompt を短めにする
  - 主役、構図、背景、小物 1〜2 個、画風、文字なしを優先
  - hidden detail を詰め込みすぎない
- `pro_consistent`
  - motif / hidden detail / background richness をより活かす
  - consistency rule を強く入れる
- `kontext_reference`
  - 強い参照画像ベース検証用 profile として扱う

### 参照画像を全ページで使う検証

ページ間の顔・髪型・服装の揺らぎを減らすため、`characterConsistencyMode: "all_pages"` の検証を行う。

- `standard_paid` / `premium_paid` は `all_pages` を優先
- `free` / `light_paid` は当面 `key_pages`
- 将来的に原価と品質のバランスを見て再調整する
