...
- スタイルカード画像は UI 上のサンプルであり、通常生成では参照画像として使わない。
- 生成時の絵柄制御は `styleBible` とスタイル用テキスト指示で行い、参照画像は原則として child protagonist / storyCast の一貫性維持に限定する。
- seed は絵柄指定ではなく、同条件比較と再現性確認のために使う。
- 通常の絵本ページでは、背景を1か所に固定しない。背景はページの役割、感情、時間経過、発見を支えるために柔軟に変化してよい。
- 避けるべきなのは背景の変化そのものではなく、不自然物、読める文字、ロゴ、危険物、scene と矛盾する物体、storyGoal と無関係なノイズである。
...
- `guided_ai` / `original_ai` の通常絵本ページは `story_flexible` を基本にする。
- `story_flexible` では、各ページの `pageVisualRole` と `storyGoal` に合う情景を選ぶ。
- `opening_establishing` では場所と空気感を広く見せ、`discovery` では手元や足元の発見に寄り、`action` では探索範囲を自然に広げ、`quiet_ending` では余韻のある場所へ移ってよい。
- hidden detail は背景の小さな発見としてのみ扱い、本文や主目的に昇格させない。
- `fixed_template` やプロフィール生成では `fixed` または `profile_default` を使い、検証や再現性を優先して背景制約を強めてもよい。

### 背景制約の分離

- 子どもプロフィール画像の `basePrompt` に含まれる背景固定、構図固定、距離固定の制約は、そのまま絵本本文ページに持ち込まない。
- 絵本本文ページでは、キャラ一貫性は `characterBible`、`outfit`、`signatureItem`、`storyCast`、`reference image` で担保する。
- scene 側では「そのページの物語に自然か」を優先し、看板、文字、ブランド、危険物、不自然な乗り物や道路などを避ける。
...
- premium_paid / original_ai では story model に `gemini-2.5-pro` を優先し、本文 rewrite pass を必ず実行する。
- protagonist cast は Gemini の自由生成をそのまま使わず、childProfileSnapshot の `characterBible / outfit / signatureItem / referenceImageUrl / approvedImageUrl` を優先して正規化する。
- storyCast の主人公情報と child profile が矛盾する場合は、child profile を正とする。
- child profile の `basePrompt` に背景制約がある場合は、最終 imagePrompt でもそれを優先し、sandbox 固定なら slide / swing / playground equipment / buildings / roads / signs を入れない。
- Firestore に保存する storyQualityReport は rewrite 後の最終本文を対象にしたものを使い、Book summary と pages[].textCharCount / textSentenceCount が整合する状態を保つ。
