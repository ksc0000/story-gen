...
```
# Premium story text notes

- `premium_paid` と `original_ai` の story model は `gemini-2.5-pro` を優先し、`flash-lite` は使わない。
- premium 本文では rewrite pass を必須にし、本文の自然さ・意味量・quest resolution を強化する。
- 3歳向け premium 本文は、1ページあたり 3〜5文、70〜140文字を最低目安とし、平均 90 文字前後を推奨する。
- 4ページ構成では `opening_establishing -> discovery -> action/emotional_closeup -> quiet_ending` の流れで、最終ページに mainQuestObject の解決を必ず書く。
- `storyGoal` と `mainQuestObject` は本文全体の主筋で、`hiddenDetail` は画像内の小さな発見要素に留める。
