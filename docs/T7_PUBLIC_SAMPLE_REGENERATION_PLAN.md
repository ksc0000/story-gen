...
| T7-5c | Live QA: Group D regression + 37/37 HTTP 200 | ✅ COMPLETE |
| T7-6 | demo.ts WebP cleanup + T7 closure | ✅ COMPLETE |
...
`src/lib/demo.ts` の `sampleImageUrl` フィールド 13 エントリを `.png` → `.webp` に置換。

| Template | Before | After |
|---|---|---|
| animals | `/images/templates/animals.png` | `/images/templates/animals.webp` |
| adventure | `/images/templates/adventure.png` | `/images/templates/adventure.webp` |
| fantasy | `/images/templates/fantasy.png` | `/images/templates/fantasy.webp` |
| bedtime | `/images/templates/bedtime.png` | `/images/templates/bedtime.webp` |
| emotional-growth | `/images/templates/emotional-growth.png` | `/images/templates/emotional-growth.webp` |
| daily-habits | `/images/templates/daily-habits.png` | `/images/templates/daily-habits.webp` |
| educational | `/images/templates/educational.png` | `/images/templates/educational.webp` |
| food | `/images/templates/food.png` | `/images/templates/food.webp` |
| seasonal | `/images/templates/seasonal.png` | `/images/templates/seasonal.webp` |
| vehicles-robots | `/images/templates/vehicles-robots.png` | `/images/templates/vehicles-robots.webp` |
| fixed-first-zoo | `/images/templates/animals.png` | `/images/templates/animals.webp` |
| fixed-bedtime-good-day | `/images/templates/bedtime.png` | `/images/templates/bedtime.webp` |
| original-ai | `/images/templates/fantasy.png` | `/images/templates/fantasy.webp` |

対応 `.webp` ファイルは `public/images/templates/` に全て実在することを確認済み。

### stale .png 参照チェック (最終)

| スコープ | 結果 |
|---|---|
| `src/**` の `sampleImageUrl *.png` | ✅ なし (T7-6 で解消) |
| `src/**` の `/images/templates/*.png` | ✅ なし |
| `functions/src/**` の `/images/templates/*.png` | ✅ なし |
| ログインページ `.png` 参照 | ✅ なし (T7-4.6 で解消済み) |
| `sampleImages` フィールドの `.png` | ✅ なし |

残存 `.png` ファイル (`public/images/templates/*.png`, `public/images/styles/*.png`) はファイルシステム上に存在するが、いずれのソースコードからも参照されない。Hosting には引き続き配信されるが機能的に使われない legacy asset。削除は今後の cleanup タスクとして行うことができる（非緊急）。
...
| `cd functions && npm test -- seed-templates` | ✅ 385/385 PASS |
| `npm run build` (frontend) | ✅ 19/19 static pages |

### コード / Firestore / Hosting 変更

| 項目 | 変更 |
|---|---|
| `src/lib/demo.ts` | ✅ `sampleImageUrl` 13 エントリ `.png` → `.webp` |
| Firestore | **変更なし** |
| Firebase Hosting 再デプロイ | **なし** (次回 deploy 時に自動反映) |
| 生成ルーティング | **変更なし** |
| OpenAI candidate/gated 動作 | **変更なし** |
...
| Slice | 内容 | 結果 |
|---|---|---|
| T7-1 | Inventory — 公開 assets 棚卸し | ✅ COMPLETE |
| T7-2 | Group C: 不足 UI イラスト・アイコン 7 枚生成 | ✅ COMPLETE |
| T7-2.5 | Group C ライブ UI 検証 | ✅ COMPLETE |
| T7-3a | Group A スタイルプレビュー設計 | ✅ COMPLETE |
| T7-3b | Group A: スタイルプレビュー 10 枚 WebP 生成 | ✅ COMPLETE |
| T7-3.5 | Group A ライブ StylePicker 検証 | ✅ COMPLETE |
| T7-4a | Group B テンプレートサムネイル設計 | ✅ COMPLETE |
| T7-4b | Group B: テンプレートサムネイル 10 枚 WebP 生成 + Firestore 更新 | ✅ COMPLETE |
| T7-4.5 | Group B ライブ ThemeCard 検証 | ✅ COMPLETE |
| T7-4.6 | stale `.png` 参照クリーンアップ (テスト + ログインページ) | ✅ COMPLETE |
| T7-5a | Group D クオリティサンプル設計 | ✅ COMPLETE |
| T7-5b | Group D: 10 枚 WebP 生成 + Firestore sampleImages 更新 + Hosting デプロイ | ✅ COMPLETE |
| T7-5c | Group D ライブ QA — 37/37 HTTP 200 PASS | ✅ COMPLETE |
| T7-6 | demo.ts WebP 統一 + T7 closure | ✅ COMPLETE |

### 最終本番検証 (T7-5c 時点)

- **Production URL**: https://story-gen-8a769.web.app
- **Group A** (style previews): 10/10 HTTP 200 image/webp
- **Group B** (template thumbnails): 10/10 HTTP 200 image/webp
- **Group C** (UI assets): 7/7 HTTP 200 image/webp
- **Group D** (quality samples): 10/10 HTTP 200 image/webp

### 残存既知事項

1. `public/images/templates/*.png` / `public/images/styles/*.png` がファイルシステムに残存 — ソースコードから参照なし。非緊急削除候補。
2. テストスイート 3 件の pre-existing 失敗 (`test/prompt-builder.test.ts` × 1, `test/test-image-models.test.ts` × 1, 他) — T7 スコープ外、別途対応。
