...
| **P5-3-execute-b** | Cohort B 限定ロールアウト実施 — P5-3c-verify PASS を受けた PM GO 判断。3–5 テスターへの限定招待。初回 1h monitoring window active。SJ/IM は引き続き disabled。招待人数上限 5 名、公開アクセス不可。 | 🟡 **GO 決定済み (2026-05-22)** — 招待準備中。[P5_COHORT_B_GO_NOGO_CHECKLIST.md](./P5_COHORT_B_GO_NOGO_CHECKLIST.md) |
...
| **P5-3f** | カバー／ページ画像モデル統一実験計画 — Cohort B フィードバック #2 per-page 調査で全 14 失敗が安全性拒否（E005 相当、参照画像トリガー）と確定。Option B（Strict）は内部診断専用に変更。**推奨 First Experiment: Option C（Safer High-Quality Retry: pro_consistent 通常 → pro_consistent 参照画像なし → klein_fast）**。Cohort B 継続可。詳細: [P5_COVER_PAGE_MODEL_UNIFICATION_EXPERIMENT.md](./P5_COVER_PAGE_MODEL_UNIFICATION_EXPERIMENT.md) | 📋 PLANNING (2026-06-03) — Option C 優先に更新済み |
