...
| **P5-3-execute** | Cohort A ソフトローンチ実施 — SLO（book readable 100%、p95 storyMs 108s）は合格。**ただし竹プランの画質 UX バグ（全ページ同じ絵・スタイル非反映）が判明、PM 判断により Cohort B はこのバグの修正／軽減まで保留。** P5-3c-verify PASS により解消確認済み。 | ✅ Cohort A 完了 (2026-05-22) — Cohort B: 🟡 **unblock candidate** (P5-3c-verify PASS 2026-05-22 — PM 判断で招待可) |
...
> - ~~ビルド・テスト全通過~~ ✅ 確認済み
>
> **→ Cohort B: unblock candidate** — PM 判断で Cohort B 招待可。招待時に `simplified_scene` 実験フラグを付与するか、本番 default に昇格するかを PM が決定する。残存リスク: 子ども写真登録済みユーザーでのキャラクター一貫性は未検証。
