...
| **P2-8** | Cloud Logging 保存クエリ定義（Log Explorer 用） | 中 |
...
| **P2-10** | CG-1 アラートポリシー定義（YAML spec + gcloud コマンド + 初動対応ランブック） | ✅ COMPLETE (docs, 2026-05-21) — [P2_CG1_CANDIDATE_GATE_ALERT_POLICY.md](./P2_CG1_CANDIDATE_GATE_ALERT_POLICY.md)；live 作成は gcloud + `roles/monitoring.alertPolicyEditor` が必要 |
| **P2-11** | ダッシュボードパネル追加 | 中 |
| **P2-12** | 通知チャンネル設定＋インシデントランブック統合（**推奨次タスク**） | 高 |
| **prod-baseline** | 本番ユーザー 7日分データで再計測、P4-15 §7.2 更新、repair retry 判断 | 高（本番稼働後） |
