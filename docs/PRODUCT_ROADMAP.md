...
| **P2-10b-enable** | 本番 baseline ≥ 30 books 後に SJ/IM ポリシーの閾値チューニングと有効化 | **推奨次タスク** |
...
| **P5-3-execute** | Cohort A ソフトローンチ実施 — SLO（book readable 100%、p95 storyMs 108s）は合格。**ただし竹プランの画質 UX バグ（全ページ同じ絵・スタイル非反映）が判明、PM 判断により Cohort B はこのバグの修正／軽減まで保留。** | ✅ Cohort A 完了 (2026-05-22) — Cohort B: 🔴 HOLD（竹プランバグ解消まで）|
...
| **P5-3c** | 竹プラン画質ブロッカー ゲート付き実験 — カバー生成パスとページ生成パスの比較調査＋実験実装。`generationOverride.p5PageExperiment === "simplified_scene"` をユーザー doc に設定した管理者/QA テスターのみ有効。実験内容: キャラクターバイブルなし・参照画像なしのカバースタイル短縮プロンプトでページ画像を生成し、スタイル反映・ページ多様性・キャラクター一貫性の影響を検証。診断ログ `cover_vs_page_profile_diagnostic` / `p5_page_experiment_active` 追加。 | ✅ COMPLETE (2026-05-22) — テスト 1604 件全通過。手動 QA 煙テスト待ち |
| **P5-3c-verify** | `simplified_scene` 実験の手動 QA 確認 — 5 ケース（竹×4テーマ＋梅 regression）を生成し目視確認。スクリプト: `set-p5-page-experiment.js` → `create-p5-3c-verify-books.js` → `print-p5-3c-verify-log-queries.mjs`. 合格条件: ページごとに異なる絵、スタイル反映、キャラクター一貫性は可（支配しない）、`page_image_failed = 0`, `duplicate_page_image_urls_detected` なし, `image_prompt.all_identical` なし。 | ⏳ 手動 QA 待ち |
| **P5-4** | prod-baseline 再計測 — ≥ 30 real `book_outcome` 蓄積後に再実行、P4_PERMANENT §7.N 更新 | ⏳ 本番トラフィック蓄積後 |
| **P5-5** | P2-10b-enable — SJ/IM アラートポリシー閾値チューニング＋有効化（prod-baseline 確定後のみ） | ⏳ P5-4 完了後 |

> **Cohort B HOLD 解除条件（P5-3c 実験検証完了基準）**
> - `simplified_scene` 実験で各ページが異なる絵になることを 2 テーマ以上で目視確認
> - 選択したスタイルが視覚的に反映されている
> - `page_image_failed = 0`（成功ケース）
> - `duplicate_page_image_urls_detected` / `image_prompt.all_identical` が出ない
> - ビルド・テスト全通過（✅ 確認済み）
> - 上記確認後 PM 判断で Cohort B 招待を手動送付（実験モードは外して本番 default に反映するか、そのまま適用するかを PM が決定）
