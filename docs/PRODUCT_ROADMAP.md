...
| **P5-3f** | カバー／ページ画像モデル統一実験計画 — Cohort B フィードバック #2 調査でカバー（pro_consistent）とページ（7/8 が klein_fast フォールバック）の品質ギャップを特定。3 オプション（Strict 廃止 / Safer Retry / Candidate 比較）の実験計画策定。推奨 First Experiment: Option B（Strict Unification、候補ゲート限定）。詳細: [P5_COVER_PAGE_MODEL_UNIFICATION_EXPERIMENT.md](./P5_COVER_PAGE_MODEL_UNIFICATION_EXPERIMENT.md) | 📋 PLANNING (2026-06-03) — docs only |
