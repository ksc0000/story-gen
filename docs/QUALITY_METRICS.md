...
Detailed Rubric: [RUBRIC_STORY_QUALITY.md](./RUBRIC_STORY_QUALITY.md)
...
Detailed Rubric: [RUBRIC_ILLUSTRATION_QUALITY.md](./RUBRIC_ILLUSTRATION_QUALITY.md)
...
| artifact avoidance | 10 | 破綻・不要文字（看板・ポスター・バナー等）・手足崩れ・不自然な物体を避けているか |
| **Total** | **100** |  |
...
| artifact avoidance | 破綻や不要文字（看板・ポスター・バナー等）が目立たない | 軽微な artifact がある | 手足・顔・文字・物体破綻が目立つ |
...
Detailed Rubrics:
- [RUBRIC_CHARACTER_CONSISTENCY.md](./RUBRIC_CHARACTER_CONSISTENCY.md) (Visual stability)
- [UNINTENDED_CHARACTER_RUBRIC.md](./QUALITY_RUBRICS/UNINTENDED_CHARACTER_RUBRIC.md) (Extra characters & duplication)
...
Detailed Rubric: [RUBRIC_PERSONALIZATION.md](./RUBRIC_PERSONALIZATION.md)
...
Detailed Rubric: [RUBRIC_SAFETY.md](./RUBRIC_SAFETY.md)
...
| **daily-life** (毎日のくらし) | 日常の発見や気づきの新鮮さ、子どもの視点から見た「ふつう」の豊かさ |
...
- **daily-life**:
  - High: 子どもの日常の「当たり前」を新鮮な視点で捉え、親も「そうそう」と共感できる描写。
  - Medium: 日常の場面は描かれているが、発見や気づきが薄く、印象に残りにくい。
  - Low: 描写が平凡すぎて絵本としての魅力がない、または日常から乖離している。

---

## ページ役割別 Illustration 品質要件

ページ役割（`pageVisualRole`）ごとの期待される視覚的品質。`imagePurpose` は生成モード（`book_cover` / `book_page`）を指し、ページ内の役割は `pageVisualRole` で区別する。
